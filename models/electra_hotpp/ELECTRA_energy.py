...
class ELECTRA(L.LightningModule, IOMixIn):
    ...
    def __init__(
        ...
        self.use_l1_energy_features = config.get('use_l1_energy_features', False)
        self.use_extra_mlp = config['use_extra_mlp']
        self.l1_energy_out_dim = config.get('l1_energy_out_dim', config['hotpp_outdim'])
        ...
        energy_readout_in = (self.units if not self.use_extra_mlp else self.energy_dim) + (self.l1_energy_out_dim if self.use_l1_energy_features else 0)
    ...
    def forward(self,
        ...
        if self.use_l1_energy_features:
            vec_channels = X_vec.view(n_atoms, self.units, 3)
    ...
    def training_step(self,
        ...
        mae = torch.abs(predicted_energy - target_energy)
        ...
        metrics = {
            "Train Energy Loss": (loss.detach(), True),
    ...
    def validation_step(self,
        ...
        metrics = {
            "Validation Energy Loss": (loss.detach(), True),
    ...
    def test_step(self,
        ...
        metrics = {
            "Test Energy Loss": (loss.detach(), True),
